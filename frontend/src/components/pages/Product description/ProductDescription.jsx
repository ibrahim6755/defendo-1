...
import { useGetRingDetailsByIdQuery } from '../../../features/rings';
...
function ProductDescription() {
    ...
    const { id } = useParams();
    const { data: productData } = useGetRingDetailsByIdQuery(id)
                        ...
                        <select name="" id="" className='w-100 p-1' value={selectedSize} onChange={(e) => setSelectedSize(e.target.value)}>
                            {
                                productData?.variants.map((variant, idx) => {
                                    if (variant.stock > 0) {
                                        return (
                                            <option key={idx} className=''>{variant.size}" inches</option>
                                        );
                                    }
                                    return null;
                                })
